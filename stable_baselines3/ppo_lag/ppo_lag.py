...
class PPOLagrangian(OnPolicyWithCostAlgorithm):
    ...
    def train(self) -> None:
        ...
        rw_policy_losses, ct_policy_losses = [], []
        clip_fractions = []
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                clipped_ration = th.clamp(ratio, 1 - clip_range, 1 + clip_range)
                policy_loss_2 = reward_advantages * clipped_ration
                rw_policy_loss = -th.min(policy_loss_1, policy_loss_2).mean()

                # Add cost to loss
                current_penalty = self.dual.nu().item()
                ct_policy_loss1 = cost_advantages * ratio
                ct_policy_loss2 = cost_advantages * clipped_ration
                ct_policy_loss = th.max(ct_policy_loss1, ct_policy_loss2).mean()

                policy_loss = rw_policy_loss + current_penalty * ct_policy_loss
                policy_loss /= (1 + current_penalty)

                # Logging
                rw_policy_losses.append(rw_policy_loss.item())
                ct_policy_losses.append(ct_policy_loss.item())
        ...
        average_action_norm = np.mean(np.linalg.norm(self.rollout_buffer.actions, axis=1, ord=np.inf))

        if self.update_penalty_after is None or ((self._n_updates/self.n_epochs) % self.update_penalty_after == 0):
            #TODO: to avoid large nu updates, the maximum average cost is restricted
            average_cost = min(average_cost, 0.2)
        ...
        logger.record("train/policy_reward_loss", np.mean(rw_policy_losses))
        logger.record("train/policy_cost_loss", np.mean(ct_policy_losses))
        ...
        logger.record("train/mean_reward", average_reward)
        logger.record("train/mean_action_norm", average_action_norm)
